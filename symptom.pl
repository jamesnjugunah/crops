...
get_plant_part(PlantPart) :-
    write('Enter the affected plant part (Choose from:leaves,leaf,ears,foliages,stem,foliage : '),
    read(PlantPart).

get_symptom(Symptom) :-
    write('Enter the observed symptom (Choose from:yellow_spots, orange_pustules, dark_spots, yellow_halo_around_spots, swollen_galls, dark_galls, brown_streaks, wilting_leaves, yellow_streaks_in_the_sun, stunted_growth, mottled_leaves): '),
    read(Symptom).
